...
The current command set is `start, assets, send, predict, sell, buy, markets, checkin, language` — though only `start, assets, send, predict, language` appear in the `/` menu (`sell`/`buy`/`markets`/`checkin` still work when typed but are hidden from `command_menu`). `/assets` shows the caller's coins (fruit display is **hidden until the fruit feature is designed** — the `fruit` column still exists and `/send`/`/sell`/`/buy` still move fruit, it's just not rendered) **plus their open positions** in two sections, each omitted when empty:
