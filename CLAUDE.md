...
3. `Confirm` (`szc:<qid>:<outcome>:<total>`) shows a **confirmation screen** (the
   ...
   odds, not the snapshot the user was viewing. It then debits `total × COIN` via
   `balance_change` and records the wager (`Database::place_wager`) with those
   **just-fetched** `odds_cents`. Payout on a win is
