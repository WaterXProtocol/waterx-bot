...
- **`inv:link` — copyable link**: `handle_invite_link` **edits** the message to the referral link in a tap-to-copy `<code>` span (`tg::edit_html_with_buttons`, HTML `parse_mode`) + `[⬅ Back]` (→ `menu:invite`, the chooser).
- **`inv:fwd` — forwardable message**: `handle_invite_fwd` **edits** the message to a friendly one-liner (`i18n::invite_forward`) with the link in **plain text** (so it survives a forward — inline keyboards don't) **plus the `[🎮 Play now]` URL deep-link button** (`menu::referral_link`, needs `BotUsernameKey`) and `[⬅ Back]`.
