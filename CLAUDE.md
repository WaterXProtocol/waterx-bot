...
- **`inv:fwd` — forwardable message**: `handle_invite_fwd` sends a friendly one-liner (`i18n::invite_forward`) with the link in **plain text**, so it survives a forward (inline keyboards don't).
- **`inv:qr` — QR code**: `handle_invite_qr` sends a **single photo** — a QR of the link (generated **locally** via `qrcode_generator::to_png_to_vec`, no third-party service), caption = link + referral count (`i18n::invite_text`), keyboard = `[🎮 Play now]` URL deep-link button (`menu::referral_link`, needs `BotUsernameKey`). The QR is identical every tap (the link is immutable), so Telegram's returned **`file_id`** is cached per user (`callbacks::qr_cache`, in-memory `OnceLock<parking_lot::Mutex<HashMap<i64,String>>>`) and re-sent via `tg::send_photo_id` with no regeneration/upload on later taps; only the first tap (or first after a restart) uploads via `tg::send_photo_bytes`, which returns the `file_id`. (A bot can't read arbitrary DM media, but it can re-send its own uploads by `file_id`.) `tg::send_photo_bytes` posts the multipart `sendPhoto` directly via `reqwest` (token from `util::bot_token`) rather than telexide's `send_photo` — **telexide's file-upload path is broken**: it serialises the `photo` field as `attach://qr.png` but names the multipart part `qr` (truncated at the first `.`), so Telegram never matches the attachment and the upload silently fails (this also lets us attach a `reply_markup`, which telexide's button struct can't — see `tg.rs`'s header). On forwarding, Telegram **strips the entire inline keyboard** (callback *and* `url` buttons), so a forwarded QR relies on the **caption link text** and the **QR image** (link baked in), not the button. The QR path is best-effort: on any failure it falls back to `tg::send_with_buttons` (link + count text with the `[Play]` button). **Referral system** (three surfaces, one binding/payout path — shared payout in `referral::pay_referral`, which credits **both** sides `referral::REFERRAL_REWARD` = 10 and DMs the referrer via `i18n::referral_bonus`):
