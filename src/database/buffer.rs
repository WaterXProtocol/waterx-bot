...
impl Database {
    ...
    pub fn claim_envelope(&self, chat: i64, msg: i64, user: i64) -> SqlResult<Option<i64>> {
        self.with_tx(|tx| {
            ...
            if deleted != 1 {
                return Ok(None); // lost the race → empty commit, no credit
            }
    ...
    pub fn open_sell_offer(
        ...
        self.with_tx(|tx| {
            ...
            if escrowed.is_empty() {
                return Ok(escrowed); // empty commit (only the harmless ensure_row insert)
            }
    ...
    pub fn open_buy_offer(
        ...
        self.with_tx(|tx| {
            ...
            if bal < price {
                return Ok(false); // empty commit (only the harmless ensure_row insert)
            }
    ...
    pub fn consume_sell(&self, chat: i64, msg: i64, taker: i64) -> SqlResult<OfferOutcome> {
        self.with_tx(|tx| {
            ...
            if bal < price {
                return Ok(OfferOutcome::TakerNotEnoughBalance); // empty commit (== rollback)
            }
        ...
        self.with_tx(|tx| {
            ...
            for f in fruits.chars() {
                ...
                if let Some(idx) = sf_check.find(&s) {
                    ...
                    return Ok(OfferOutcome::TakerMissingFruit(f)); // empty commit (== rollback)
                }
            }
